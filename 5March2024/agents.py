...
def app():
    ...
    # DONUT CHART
    st.markdown("### Best Selling Agents")
    fig2 = px.pie(filtered_df, values='Supplier Amount', names='Agent Name', hole=0.5,
        labels={'Supplier Amount': 'Supplier Total Price', 'Agent Name': 'Agents'})
    # Display the donut chart
    st.plotly_chart(fig2)
    
    # Create the scatter plot with specified colors
    fig3 = px.scatter(filtered_df, x='Agent Name', y='Supplier Amount', color='Agent Name', 
                    labels={'Supplier Amount': 'Supplier Total Price', 'Agent Name': 'Agents'},
